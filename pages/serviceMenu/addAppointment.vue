            ...
            <form @submit.prevent="submitAppointment" ref="appointmentForm">
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 mb-4">
                        <label class="label">{{
                            $t("workingTime.available_date")
                        }}</label>
                        <div
                            class="main_input"
                            :class="{
                                'is-invalid': showValidation && !dateValue,
                            }"
                        >
                            <flat-pickr
                                v-model="dateValue"
                    ...
                    <div class="col-12 col-md-6 mb-4">
                        <div class="special_sppiner position-relative">
                            <GlobalCustomDropdown
                                ...
                                option-label="time_text"
                                :placeholder="$t('workingTime.choose_time')"
                                :label="$t('workingTime.available_time')"
                                :showValidation="showValidation"
                                :validationSchema="validations.time"
                                :disabled="!dateValue || loadingTimeOptions"
                            />

                            <div
                                v-if="loadingTimeOptions"
                    ...
                    <FormInput
                        ...
                        type="number"
                        min="1"
                        :label="$t('workingTime.number_of_people')"
                        :placeholder="$t('workingTime.number_of_people')"
                    ...
                    <div class="col-12 mb-4">
                        <GlobalCustomDropdown
                            ...
                            option-value="id"
                            option-label="name"
                            :placeholder="$t('workingTime.choose_employee')"
                            :label="$t('workingTime.employee_name')"
                            :showValidation="showValidation"
                    ...
                    <div class="form-group">
                        ...
                        <div class="position-relative with_icon">
                            ...
                            ></textarea>
                            <img
                                src="@/assets/images/file-img.svg"
                                alt="icon"
                                class="input-icon with-area"
                            />
                        </div>
                        <p
                            v-if="notesError"
...
const serviceId = ref(route.query.serviceId || null);

const pageHeadTitle = ref(t("sideMenu.list_reservations"));
// Set global store
globalStore.title = pageHeadTitle.value;
globalStore.titleIcon = "fa-solid fa-angle-left";
globalStore.titleLink = "/serviceMenu";
globalStore.subtitle = t("workingTime.add_appointment_title");

// Axios
const axios = useApi();

// Validation schemas
const { customerName, phoneNumber, numberOfPeople, Notes, date, time, required } =
    useValidationSchema();
...
const validations = {
    ...
    employee: required(t("workingTime.employee_name")),
    customerName: customerName(t("workingTime.customer_name")),
    phoneNumber: phoneNumber(t("workingTime.phone_number")),
    numberOfPeople: numberOfPeople(t("workingTime.number_of_people")),
...
const availableEmployeeOptions = ref([]);
...
const fetchAvailableTimeSlots = async (date) => {
    if (!date || !serviceId.value) return;
    ...
    try {
        const res = await axios.get(
            `provider/services/${serviceId.value}/available-hours?date_at=${date}`,
            config.value,
        );

        if (res.data.key === "success") {
            // Filter only available times (not reserved)
            availableTimeOptions.value = res?.data?.data?.available_hours
                ...
                .map((slot) => ({
                    ...
                    name: slot.time_text, // للعرض في الـ dropdown
                }));
        }
...
const fetchAvailableEmployees = async () => {
    ...
    availableEmployeeOptions.value = [];

    try {
        const res = await axios.get(
            `provider/services/${serviceId.value}/available-employees`,
            config.value,
        );

        if (res.data.key === "success") {
            availableEmployeeOptions.value = res?.data?.data?.employees || [];
        }
        loadingEmployeeOptions.value = false;
    } catch (error) {
        console.error("Fetch employees error:", error);
        errorToast("حصل خطأ في تحميل الموظفين");
    } finally {
        loadingEmployeeOptions.value = false;
    }
};
...
const submitAppointment = async () => {
    ...
    try {
        ...
        fd.append("date_at", dateValue.value);
        const rawTime =
            typeof timeValue.value === "object"
                ? timeValue.value.value
                : timeValue.value;
        ...
        fd.append("time_value", formattedTime);
        
        // Add employee_id if selected
        if (employeeValue.value) {
            fd.append("employee_id", employeeValue.value);
        }

        const res = await axios.post(
            `provider/services/${serviceId.value}/store-external-reservation`,
            fd,
            config.value,
        );
        if (res.data.key === "success") {
