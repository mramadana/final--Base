<template>
  ...
  <div class="main-table">
    <DataTable :value="products" :filters="filters" :rows="rows" :sortable="sortable" tableStyle="min-width: 50rem"
      class="main-table" id="my_table" ref="my_table">
      <template #empty>
        {{ $t("Table.no_records_found") }}
      </template>

      <div v-for="col in columns" :key="col.field">
        <Column :field="col.field" :header="col.header" :sortable="sortable" v-if="col.field !== 'abilities'">
          <template #body="slotProps">
            <!-- Image -->
            <!-- <img v-if="col.field === 'image' && showImage" :src="slotProps.data.image" loading="lazy"
              class="shadow-4 datatable-image-custom" /> -->

            <Image v-if="col.field === 'image' && showImage" :src="slotProps.data.image" loading="lazy" class="shadow-4 datatable-image-custom" preview />

            <!-- Status badges -->
            <span v-else-if="col.field === 'status'" :class="['status-badge', `status-${slotProps.data.statusBadge}`]">
            ...
            <span v-else-if="col.field === 'reservations'" class="chip chip-dark"
        ...
        <template #body="slotProps">
          <div class="dropdown drop-lang dropdown-table sort-table">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              ...
              <li>
                <router-link :to="'/branches/' + slotProps.data.id + '/edit'" class="table_link dropdown-item"
              ...
              <li>
                <button class="dropdown-item" @click="deleteItem(slotProps.data.id)">
                  {{ $t("Cars.delete") }}
              ...
              <li>
                <router-link :to="'/branches/branchdata'" class="table_link dropdown-item"
          ...
          <div class="dropdown drop-lang dropdown-table sort-table">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              ...
              <li>
                ...
                <router-link :to="DropDownManagers.path + '/' + DropDownManagers.subroute"
                  class="table_link dropdown-item" @click="editItem(slotProps.data.id)">
                  {{ $t("Cars.edit") }}
                  <i class="far fa-edit custom-icon delete"></i>
                </router-link>
              ...
              <li>
                <router-link :to="DropDownManagers.path + '/' + slotProps.data[DropDownManagers.id]"
                  class="table_link dropdown-item" @click="viewItem(slotProps.data.id)">
                  {{ $t("Cars.view_data") }}
                  <i class="fas fa-eye custom-icon details"></i>
                </router-link>
              </li>
    ...
    <Dialog v-model:visible="showReservationsDialog" modal :draggable="false" class="custum_dialog_width">
      ...
      <div class="times-grid">
        <button v-for="(item, i) in reservationsList" :key="i" type="button"
