<template>
    <div class="main-table">
        ...
        >
            ...
            <div v-for="col in columns" :key="col.field">
              ...
              class="shadow-4 datatable-image-custom" /> -->
                        ...
                        <span
                            ...
                            :class="[
                                'status-badge',
                                `status-${slotProps.data.statusBadge}`,
                            ]"
                        >
                        ...
                        <!-- Status text (e.g. settlement: مقبول / مرفوضة) with color by status -->
                        <span
                            v-else-if="
                                col.field === 'status_text' &&
                                slotProps.data.status
                            "
                            :class="[
                                'status-badge',
            ...
            <!--*********** Actions Column (Slot) ***********-->
            <Column
                :header="actionsHeader || routeTable?.header"
                v-if="$slots.actions || routeTable"
            >
                <template #body="slotProps">
                        ...
                        >
                            <li>
                                <router-link
                                    :to="
                                        ...
                                        '/edit'
                                    "
                                    class="table_link dropdown-item"
                                    @click="editItem(slotProps.data.id)"
                                >
                                    {{ $t("Cars.edit") }}
                                    <i
                                        class="far fa-edit custom-icon delete"
                                    ></i>
                                </router-link>
                            </li>
                            ...
                            <li>
                                ...
                                >
                                    {{ $t("Cars.delete") }}
                                    <i
                                        class="fas fa-trash-alt custom-icon edit"
                                    ></i>
                                </button>
                            </li>
                            ...
                            <li>
                                <router-link
                                    :to="'/branches/branchdata'"
                                    class="table_link dropdown-item"
                                    @click="viewItem(slotProps.data.id)"
                                >
                                    {{ $t("Cars.view_data") }}
                                    <i
                                        class="fas fa-eye custom-icon details"
                                    ></i>
                                </router-link>
                            </li>
                        </ul>
                    ...
                    <div class="dropdown drop-lang dropdown-table sort-table">
                        ...
                        >
                            <li>
                                <router-link
                                    :to="
                                        DropDownManagers.path +
                                        '/' +
                                        DropDownManagers.subroute
                                    "
                                    class="table_link dropdown-item"
                                    @click="editItem(slotProps.data.id)"
                                >
                            ...
                            <li>
                                <router-link
                                    :to="
                                        DropDownManagers.path +
                                        '/' +
                                        slotProps.data[DropDownManagers.id]
                                    "
                                    class="table_link dropdown-item"
                                    @click="viewItem(slotProps.data.id)"
                                >
        ...
        <!-- Reservations Dialog -->
        <Dialog
            v-model:visible="showReservationsDialog"
            modal
            :draggable="false"
            class="custum_dialog_width"
        >
            <h3 class="main-title md mb-4">
                {{ $t("tables.reservations_list") }}
            </h3>
