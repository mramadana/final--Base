<template>
    <div>
                ...
                <div v-for="(item, index) in calibrationData" :key="item.id || index" class="col-lg-4 col-md-6 col-12 mb-3">
                    <div class="d-flex align-items-center justify-content-between payment-field h-100">
                        ...
                        <div class="check-text hint d-flex">
                            <div class="main-title mb-0 fz-13">
                                {{ item.name }}
                            </div>
                        </div> 
        ...
        <button class="custom-btn md mt-4" @click="saveCalibration">
            ...
            <span class="spinner-border spinner-border-sm" v-if="saving" role="status"
    ...
    try {
        ...
        if (response(res) === "success") {
            ...
            setTimeout(() => {
                ...
                // Emit event to close parent dialog
                emit('close-dialog');
            }, 2000);
        } else {
    ...
    try {
        ...
        if (response(res) === "success") {
            ...
            calibrationData.value = standards.map(standard => ({
                name: standard.name || standard.title || standard.calibration,
...
defineExpose({
    selectedItems: computed(() => calibrationData.value.filter(item => item.selected)),
    calibrationData,
    loadCalibrationData,
    saveCalibration
});
